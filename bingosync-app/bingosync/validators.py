...
def validate_room_name(value):
    ...
    if not value or not value.strip():
        raise ValidationError("Room name cannot be empty or contain only whitespace.")
    ...
    if len(value) > MAX_ROOM_NAME_LENGTH:
        raise ValidationError(f"Room name cannot exceed {MAX_ROOM_NAME_LENGTH} characters.")
...
def validate_player_name(value):
    ...
    """
    if not value or not value.strip():
        raise ValidationError("Player name cannot be empty or contain only whitespace.")
    ...
    if len(value) > MAX_PLAYER_NAME_LENGTH:
        raise ValidationError(f"Player name cannot exceed {MAX_PLAYER_NAME_LENGTH} characters.")
...
def sanitize_text_input(value):
    ...
    # Remove control characters (except newlines and tabs for multi-line fields)
