...
class HPCoSGeneratorIntegrationTestCase(TestCase):
    ...
    def test_board_generation_produces_25_goals(self):
        ...
        for i, goal in enumerate(card):
            ...
            self.assertIsInstance(goal['name'], str, f"Goal {i} name should be a string")
            self.assertIsInstance(goal['tier'], int, f"Goal {i} tier should be an integer")
            self.assertGreater(len(goal['name']), 0, f"Goal {i} name should not be empty")
    ...
    def test_different_seeds_produce_different_boards(self):
        ...
        differences = sum(1 for g1, g2 in zip(card1, card2) if g1['name'] != g2['name'])
        self.assertGreater(
            differences, 
            0, 
    ...
    def test_all_difficulty_tiers_represented(self):
        ...
        self.assertGreaterEqual(
            len(all_tiers),
            15,
            f"Should have at least 15 different difficulty tiers across {num_boards} boards, got {len(all_tiers)}"
        )
    ...
    def test_variable_amounts_randomize_correctly(self):
        ...
        for i in range(5):
            ...
            # Look for goals with numbers (indicating variable amounts were processed)
            for goal in card:
                # Check if goal name contains numbers (e.g., "3 chocolate frogs")
    ...
    def test_no_javascript_errors_during_execution(self):
        ...
        for seed in seeds:
            try:
                seed_result, card = self.generator.get_card(seed=seed)
                self.assertEqual(len(card), 25, f"Seed {seed} should produce 25 goals")
    ...
    def test_goal_names_are_valid_strings(self):
        ...
        for i, goal in enumerate(card):
            ...
            # Should not be empty
            self.assertGreater(len(name), 0, f"Goal {i} name should not be empty")
            
            # Should not be just whitespace
            self.assertGreater(len(name.strip()), 0, f"Goal {i} name should not be just whitespace")
    ...
    def test_difficulty_tiers_are_valid(self):
        ...
        for i, goal in enumerate(card):
            ...
            self.assertIsInstance(tier, int, f"Goal {i} tier should be integer")
    ...
    def test_extreme_seed_values(self):
        ...
        for seed in extreme_seeds:
            ...
            self.assertIsNotNone(seed_result, f"Seed {seed} should return seed value")
